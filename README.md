...
推荐用自带的补图工具，浏览器里拖图片，不用自己剪裁/编辑 YAML：

```bash
pnpm run photo-admin   # → http://localhost:4322，只在本机跑，不随网站部署
```

打开后选食材、把图拖进对应的图片位（成品特写 / 选材参考图 / 穿串手法图 / 熟度判断图），填一行替代文字，点上传——工具会自动把图**压缩到长边 2000px**、存进 `src/assets/bbq/<slug>/`，并把 `photo`/`photoAlt` 写进对应食材的 `.md`。**图片直接落在本地磁盘、进 git 仓库，不经过任何对象存储/云服务。** 传完自己确认一下（`pnpm dev` 看一眼），照常 `git add -A && git commit && git push`——这个工具不会替你提交。

也可以手动来：图片放 `src/assets/bbq/<slug>/`，在对应食材的 `.md` 里填 `photo`（相对路径，如 `../../assets/bbq/wuhua-rou/done.jpg`）和 `photoAlt`（替代文字），推送即可，构建期会自动压缩、转 webp、生成宽高属性——但这样**手机原图要自己先缩到长边 2000px 以内**（用补图工具就不用操心这步，它会自动做）。原图动辄 3–5MB，一旦进了 git 历史就永远留在仓库里，删不干净。
